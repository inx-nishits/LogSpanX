      ...
      <div className="w-full bg-white border-b border-[#e4eaee]">
        <div className="px-5 pt-4 pb-1">
          ...
          <div className="flex items-start justify-between mt-0.5">
            <div>
              <h1 className="text-lg text-[#333] font-normal leading-tight">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <button
                ...
                className="p-1 hover:bg-[#f2f6f8] rounded transition-colors"
              >
                <Star
                  className={`h-4 w-4 ${isFavorite ? 'text-[#f5a623] fill-[#f5a623]' : 'text-[#ccc]'
                    }`}
                />
              </button>
              <button
        ...
        {/* ─── Tabs ─── */}
        <div className="px-5 flex items-center gap-0 mt-2">
          {TABS.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 text-[12px] font-semibold uppercase tracking-widest transition-colors border-b-2 ${activeTab === tab
                ? 'text-[#333] border-[#333]'
  ...
  return (
    <div ref={ref}>
      <button
        ref={btnRef}
        onClick={handleOpen}
        className="inline-flex items-center gap-1 text-[13px] cursor-pointer max-w-[260px]"
      >
        {assignees.length === 0 ? (
          <span className="inline-flex items-center gap-1 text-[#aaa] text-[13px] hover:text-[#888] transition-colors">
      ...
      {open && (
        ...
        >
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b border-[#eee]">
            <Search className="h-3 w-3 text-[#bbb] flex-shrink-0" />
            <input
              autoFocus
              value={search}
          ...
          <div className="max-h-[200px] overflow-y-auto py-0.5">
            {filtered.length === 0 ? (
              <p className="text-[13px] text-[#aaa] py-3 text-center">No members found</p>
            ) : filtered.map(member => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-2 w-full px-2.5 py-1.5 hover:bg-[#f5f7f9] transition-colors cursor-pointer"
                >
                  <div className="w-6 h-6 rounded-full bg-[#03a9f4] flex items-center justify-center text-[10px] font-bold text-white uppercase flex-shrink-0">
                    {member.name.split(' ').map(n => n[0]).join('').substring(0, 2)}
                  </div>
                  <span className="flex-1 text-[13px] text-[#333] truncate text-left">{member.name}</span>
                  {checked && <Check className="h-3 w-3 text-[#03a9f4] flex-shrink-0" />}
                </button>
              )
            })}
          </div>
          {assignees.length > 0 && (
            <div className="border-t border-[#eee] px-2.5 py-1.5">
              <button
                onClick={() => onUpdate(taskId, [])}
                className="text-[12px] text-[#f44336] hover:underline cursor-pointer"
              >
                Remove all
              </button>
            </div>
...
function TasksTab({
  ...
  return (
    <div>
      {/* Filter bar */}
      <div className="flex items-center justify-between mb-0 bg-white border border-[#e4eaee] rounded-t-md px-3 py-2">
        <div className="flex items-center gap-2">
          <div className="relative">
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value as 'active' | 'done' | 'all')}
              className="appearance-none bg-white border border-[#c6d2d9] rounded-sm px-2.5 py-[5px] pr-7 text-[13px] text-[#666] outline-none focus:border-[#03a9f4] cursor-pointer"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="pl-8 pr-3 py-[5px] text-[13px] border border-[#c6d2d9] rounded-sm outline-none focus:border-[#03a9f4] w-[200px]"
            />
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="bg-white border border-t-0 border-[#e4eaee] rounded-b-md overflow-hidden">
        <div className="bg-[#f0f7fb] px-4 py-2 border-b border-[#d6e5ef]">
          <span className="text-[13px] font-bold text-[#5c7b91] uppercase tracking-widest">
            Tasks
          </span>
        </div>

        <div className="flex items-center border-b border-[#e4eaee] px-4 py-2">
          <div className="flex-1 flex items-center gap-1 cursor-pointer">
            <span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">NAME</span>
            <ChevronDown className="h-3 w-3 text-[#ccc]" />
          </div>
          <div className="w-[300px]">
            <span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">ASSIGNEES</span>
          </div>
          <div className="w-[40px]" />
        </div>

        {filtered.length === 0 ? (
          <div className="px-4 py-10 text-center text-[#999] text-[13px]">
            No tasks found
          </div>
        ) : (
          filtered.map((task) => (
            <div
              key={task.id}
              className="flex items-center border-b border-[#f1f4f7] px-4 py-2.5 hover:bg-[#fcfdfe] transition-colors group"
            >
              {/* Complete checkbox */}
              <button
              ...
              <div className="flex-1">
                {editingId === task.id ? (
                  <input
                    ...
                    className="w-full px-2 py-1 text-[13px] border border-[#03a9f4] rounded-sm outline-none"
                    autoFocus
                  />
                ) : (
                  <span
                    className={`text-[13px] cursor-pointer ${task.completed ? 'text-[#999] line-through' : 'text-[#03a9f4]'
                      }`}
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[1000] p-4">
          <div className="bg-white rounded-sm shadow-2xl w-full max-w-[380px]">
            <div className="px-5 py-3 flex items-center justify-between border-b border-[#e4eaee]">
              <h2 className="text-[15px] font-medium text-[#333]">Delete Task</h2>
            ...
            <div className="px-5 py-3 flex items-center justify-end gap-3 border-t border-[#e4eaee]">
              <button onClick={() => setConfirmDelete(null)} className="text-[#555] text-[13px] hover:underline cursor-pointer">Cancel</button>
              <button
                onClick={confirmDeleteTask}
                className="bg-[#f44336] hover:bg-[#d32f2f] text-white px-5 py-1.5 text-[13px] font-bold rounded-sm uppercase tracking-wider cursor-pointer transition-colors"
              >
                Delete
              </button>
            </div>
...
function TaskActionMenu({
  ...
  return (
    <div
      ref={ref}
      className="fixed bg-white border border-[#e4eaee] rounded-md shadow-2xl z-[9999] py-0.5 w-[150px]"
      style={{ top: pos.top, left: pos.left }}
    >
      <button
        onClick={onRename}
        className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[12px] text-[#444] hover:bg-[#f5f7f9] transition-colors"
      >
        <Pencil className="h-3 w-3 text-[#aaa]" />
        Rename
      </button>
      <button
      ...
      >
        {isCompleted
          ...
          : <CheckCheck className="h-3 w-3 text-[#aaa]" />}
        {isCompleted ? 'Mark active' : 'Mark done'}
      </button>
      <div className="h-px bg-[#f0f4f7] my-0.5" />
      <button
        onClick={onDelete}
        className="w-full flex items-center gap-2 px-2.5 py-1.5 text-[12px] text-red-500 hover:bg-red-50 transition-colors"
      >
...
function AccessTab({
  ...
  return (
    <div className="bg-white border border-[#e4eaee] rounded-md overflow-hidden">
      {/* Visibility */}
      <div className="px-5 py-3.5 border-b border-[#e4eaee]">
        <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Visibility</h3>
        ...
        <div className="flex items-center gap-5">
          <label className="flex items-center gap-1.5 cursor-pointer">
            ...
            />
            <span className="text-[13px] text-[#333]">Private</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input
              type="radio"
              ...
              className="w-4 h-4 accent-[#03a9f4]"
            />
            <span className="text-[13px] text-[#333]">Public</span>
          </label>
        </div>
      </div>
      ...
      <div className="px-5 py-3 border-b border-[#e4eaee]">
        <button
          ...
          className="flex items-center gap-1.5 text-[#03a9f4] text-[13px] font-medium hover:underline"
        >
        ...
        {showAddPanel && (
          <div
            ref={panelRef}
            className="fixed z-[9999] bg-white border border-[#ddd] shadow-xl rounded-sm w-[270px] flex flex-col"
            style={{ top: panelPos.top, left: panelPos.left }}
          >
            {/* Search */}
            <div className="flex items-center gap-1.5 px-2.5 py-2 border-b border-[#eee]">
              <Search className="h-3 w-3 text-[#bbb] flex-shrink-0" />
              <input
                type="text"
            ...
            <div className="max-h-[200px] overflow-y-auto py-0.5">
              {availableUsers.length === 0 ? (
                <p className="text-[13px] text-[#aaa] py-4 text-center">
                ...
                availableUsers.map((user) => (
                  <label
                    key={user.id}
                    className="flex items-center gap-2 w-full px-2.5 py-2 hover:bg-[#f5f7f9] transition-colors cursor-pointer"
                  >
                    <input
                      type="checkbox"
                      checked={selectedIds.has(user.id)}
                      onChange={() => toggleSelect(user.id)}
                      className="w-3.5 h-3.5 accent-[#03a9f4] flex-shrink-0 cursor-pointer"
                    />
                    <div className="w-6 h-6 rounded-full bg-[#03a9f4] flex items-center justify-center text-[10px] font-bold text-white uppercase flex-shrink-0">
                      {user.name.split(' ').map((n: string) => n[0]).join('').substring(0, 2)}
                    </div>
                    <span className="flex-1 text-[13px] text-[#333] truncate">{user.name}</span>
                  </label>
                ))
              )}
            </div>
            ...
            {/* Footer */}
            {availableUsers.length > 0 && (
              <div className="px-2.5 py-2 border-t border-[#eee] flex items-center justify-between">
                ...
                <button
                  ...
                  className="bg-[#03a9f4] hover:bg-[#0288d1] disabled:opacity-40 text-white text-[12px] font-bold px-3 py-1 rounded-sm transition-colors cursor-pointer"
                >
      ...
      {/* Members table */}
      <div>
        <div className="bg-[#f0f7fb] px-5 py-2 border-b border-[#d6e5ef]">
          <span className="text-[13px] font-bold text-[#5c7b91] uppercase tracking-widest">
            Users
          </span>
        </div>
        <div className="flex items-center border-b border-[#e4eaee] px-5 py-2 bg-white">
          <div className="flex-1">
            <span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">NAME</span>
          </div>
          <div className="w-[250px] text-center">
            <span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">BILLABLE RATE (USD)</span>
          </div>
          <div className="w-[120px] text-right">
            <span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">ROLE</span>
          </div>
          <div className="w-[40px]" />
        </div>

        {members.length === 0 ? (
          <div className="px-5 py-10 text-center text-[#999] text-[13px]">
          ...
          members.map((member) => (
            <div
              key={member.id}
              className="flex items-center border-b border-[#f1f4f7] px-5 py-2.5 hover:bg-[#fcfdfe] transition-colors group"
            >
              <div className="flex-1">
                <span className="text-[13px] text-[#333]">{member.name}</span>
              </div>
              <div className="w-[250px] flex items-center justify-center gap-2">
                <input
                  type="text"
                  defaultValue="—"
                  className="w-[80px] px-2 py-1 text-[13px] text-center border border-[#e4eaee] rounded-sm outline-none focus:border-[#03a9f4] bg-[#f8fafb]"
                  readOnly
                />
              ...
              <div className="w-[120px] text-right">
                <span className="text-[13px] text-[#333] capitalize">
      ...
      {confirmRemove && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[1000] p-4">
          <div className="bg-white rounded-sm shadow-2xl w-full max-w-[380px]">
            <div className="px-5 py-3 flex items-center justify-between border-b border-[#e4eaee]">
              <h2 className="text-[15px] font-medium text-[#333]">Remove Member</h2>
            ...
            <div className="px-5 py-3 flex items-center justify-end gap-3 border-t border-[#e4eaee]">
              <button onClick={() => setConfirmRemove(null)} className="text-[#555] text-[13px] hover:underline cursor-pointer">Cancel</button>
              <button
                onClick={confirmUnassign}
                disabled={removing === confirmRemove.id}
                className="bg-[#f44336] hover:bg-[#d32f2f] text-white px-5 py-1.5 text-[13px] font-bold rounded-sm uppercase tracking-wider cursor-pointer transition-colors disabled:opacity-60"
              >
  ...
  return (
    <div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        {/* Stats */}
        <div className="bg-white border border-[#e4eaee] rounded-md p-5">
          <div className="border-b border-[#e4eaee] pb-3 mb-3">
            <div className="flex items-center justify-between">
              <span className="text-[12px] font-bold text-[#999] uppercase tracking-widest">TRACKED</span>
              <span className="text-[16px] font-semibold text-[#333]">{fmtHours(totalSeconds)}</span>
            </div>
          </div>
          <div className="space-y-1.5 border-b border-[#e4eaee] pb-3 mb-3">
            <div className="flex items-center justify-between">
              <span className="text-[12px] font-bold text-[#03a9f4] uppercase tracking-widest">BILLABLE</span>
              <span className="text-[14px] font-semibold text-[#333]">{fmtHours(billableSeconds)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[12px] text-[#999] uppercase tracking-widest">NON-BILLABLE</span>
              <span className="text-[13px] text-[#666]">{fmtHours(nonBillableSeconds)}</span>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[12px] font-bold text-[#999] uppercase tracking-widest">AMOUNT</span>
            <span className="text-[14px] font-semibold text-[#333]">{amount} USD</span>
          </div>
        </div>

        {/* Donut Chart */}
        <div className="bg-white border border-[#e4eaee] rounded-md p-5 flex items-center justify-center">
          <div className="relative w-[200px] h-[200px]">
            <svg viewBox="0 0 200 200" className="w-full h-full">
              ...
              <text x="100" y="96" textAnchor="middle" className="text-[13px] font-semibold fill-[#333]">
          ...
          <div className="ml-5 flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm bg-[#8bc34a]" />
              <span className="text-[13px] text-[#666]">Billable</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm bg-[#c5e1a5]" />
      ...
      {/* Tasks breakdown */}
      <div className="bg-white border border-[#e4eaee] rounded-md overflow-hidden">
        <div className="bg-[#f0f7fb] px-4 py-2 border-b border-[#d6e5ef] flex items-center justify-between">
          <span className="text-[13px] font-bold text-[#5c7b91] uppercase tracking-widest">Tasks</span>
        </div>
        <div className="flex items-center border-b border-[#e4eaee] px-4 py-2">
          <div className="flex-1"><span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">NAME</span></div>
          <div className="w-[200px]"><span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">ASSIGNEES</span></div>
          <div className="w-[120px] text-right"><span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">TRACKED</span></div>
          <div className="w-[100px] text-right"><span className="text-[12px] font-normal text-[#666] uppercase tracking-widest">AMOUNT</span></div>
        </div>
        {tasks.length === 0 ? (
          <div className="px-4 py-8 text-center text-[#999] text-[13px]">No tasks</div>
        ) : (
          tasks.map((task) => (
            <div key={task.id} className="flex items-center border-b border-[#f1f4f7] px-4 py-2.5 hover:bg-[#fcfdfe] transition-colors">
              <div className="flex-1"><span className="text-[13px] text-[#03a9f4]">{task.name}</span></div>
              <div className="w-[200px]"><span className="text-[13px] text-[#666]">Anyone</span></div>
              <div className="w-[120px] text-right"><span className="text-[13px] text-[#666]">0.00h</span></div>
              <div className="w-[100px] text-right"><span className="text-[13px] text-[#666]">0.00 USD</span></div>
            </div>
          ))
        )}
      </div>
...
function NoteTab() {
  ...
  return (
    <div className="bg-white border border-[#e4eaee] rounded-md overflow-hidden px-5 py-3">
      <textarea
        value={note}
        onChange={(e) => setNote(e.target.value)}
        placeholder="Add a note for this project..."
        className="w-full h-[180px] p-3 text-[13px] text-[#333] border border-[#c6d2d9] rounded-sm outline-none focus:border-[#03a9f4] resize-y"
      />
      <div className="flex justify-end mt-2">
        <button className="bg-[#03a9f4] hover:bg-[#0288d1] text-white text-[13px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-sm transition-colors">
          Save Note
        </button>
      </div>
    </div>
...
function SettingsTab({
  ...
  return (
    <div className="bg-white border border-[#e4eaee] rounded-md overflow-hidden">
      <div className="px-5 py-4 max-w-[500px] flex flex-col gap-5">
        {/* Name */}
        <div>
          <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Name</h3>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full px-3 py-1.5 text-[13px] border border-[#c6d2d9] rounded-sm outline-none focus:border-[#03a9f4]"
          />
        </div>
        ...
        {/* Project Lead */}
        <div>
          <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Project Lead</h3>
          <p className="text-[13px] text-[#999] mb-1.5">Used for grouping similar Projects together.</p>
          <div className="relative">
            <select
              value={leadId}
              onChange={(e) => setLeadId(e.target.value)}
              className="w-full px-3 py-1.5 text-[13px] border border-[#c6d2d9] rounded-sm outline-none focus:border-[#03a9f4] bg-white appearance-none pr-8 cursor-pointer"
            >
        ...
        {/* Color */}
        <div>
          <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Color</h3>
          <p className="text-[13px] text-[#999] mb-1.5">Use color to visually differentiate Projects.</p>
          <div className="relative">
            <button
        ...
        {/* Billable */}
        <div>
          <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Billable by default</h3>
          <p className="text-[13px] text-[#999] mb-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-[13px] text-[#666]">{billable ? 'Yes' : 'No'}</span>
          </div>
        </div>
        ...
        {/* Hourly rate */}
        <div>
          <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Project billable rate</h3>
          <p className="text-[13px] text-[#999] mb-1.5">Billable rate used for calculating billable amount for this Project.</p>
          <p className="text-[13px] text-[#666] mb-1.5">Hourly rate (USD)</p>
          <div className="flex items-center gap-2">
            <input
              type="text"
              value={hourlyRate}
              ...
              placeholder="—"
              className="w-[100px] px-3 py-1.5 text-[13px] border border-[#c6d2d9] rounded-sm outline-none focus:border-[#03a9f4] bg-[#f8fafb]"
            />
        ...
        {/* Estimate */}
        <div>
          <h3 className="text-[13px] font-semibold text-[#333] mb-0.5">Project estimate</h3>
          <p className="text-[13px] text-[#999] mb-1.5">Choose how you wish to track Project progress (time or fixed fee budget).</p>
          <div className="relative">
            <select className="w-[200px] px-3 py-1.5 text-[13px] border border-[#c6d2d9] rounded-sm outline-none focus:border-[#03a9f4] bg-white appearance-none pr-8 cursor-pointer">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="bg-[#03a9f4] hover:bg-[#0288d1] text-white text-[13px] font-bold uppercase tracking-widest px-5 py-1.5 rounded-sm transition-colors disabled:opacity-50"
          >
...
function PlaceholderTab({ name }: { name: string }) {
  return (
    <div className="bg-white border border-[#e4eaee] rounded-md px-5 py-12 text-center">
      <p className="text-[13px] text-[#999]">{name} — Coming soon</p>
    </div>
  )
}