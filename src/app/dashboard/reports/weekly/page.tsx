...
function SimpleDropdown({ value, options, onChange }: { value: string; options: string[]; onChange: (v: string) => void }) {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className="flex items-center gap-1 px-2.5 h-[26px] text-[13px] text-[#555] bg-white border border-[#d0d8de] rounded hover:border-[#aaa] cursor-pointer">
        ...
        <div className="absolute top-full left-0 mt-0.5 bg-white border border-[#ddd] shadow-lg z-50 min-w-[130px] py-0.5">
          {options.map(opt => (
            <button key={opt} onClick={() => { onChange(opt); setOpen(false) }}
              className={cn('w-full text-left px-3 py-1.5 text-[13px] cursor-pointer', value === opt ? 'bg-[#03a9f4] text-white' : 'text-[#555] hover:bg-[#f0f4f8]')}>
              {opt}
            </button>
          ))}
        </div>
  ...
  return (
    <ReportShell dateRange={dateRange} onRangeChange={setDateRange}>
      <div className="flex-1 overflow-y-auto overflow-x-auto">
        ...
        {/* Stats + actions bar */}
        <div className="flex items-center justify-between bg-white border-b border-[#e4eaee] px-5 h-[44px] min-w-[900px]">
        ...
        <div className="min-w-[900px]">
          ...
          {rows.length === 0 ? (
            ...
            rows.map(row => (
              <div key={row.id} className="flex items-center h-[44px] bg-white border-b border-[#f0f0f0] px-4 hover:bg-[#fafbfc] transition-colors">
                {/* Count */}
                <div className="w-6 flex-shrink-0 mr-2 text-[12px] text-[#aaa] tabular-nums text-center">{row.entryCount}</div>
                ...
                <div className="flex-1 min-w-0 flex items-center gap-2 pr-4">
                  ...
                  <span className={cn('text-[13px] truncate', row.billable ? '' : 'text-[#e91e63]')} style={row.billable ? { color: row.color } : {}}>
                    {row.name}
                  </span>
                  {row.lead && <span className="text-[12px] text-[#aaa] truncate">- {row.lead}</span>}
                </div>

                {/* Day columns */}
                {row.dayTotals.map((secs, i) => (
                  <div key={i} className="w-[80px] text-right flex-shrink-0 text-[13px] text-[#333] tabular-nums">
                    {fmtH(secs)}
                  </div>
                ))}

                {/* Total */}
                <div className="w-[80px] text-right flex-shrink-0 text-[13px] font-bold text-[#333] tabular-nums">
                  {fmtH(row.total)}
                </div>
              </div>
            ))
