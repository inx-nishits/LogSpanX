    ...
    return (
        <div className="bg-white border border-[#e4eaee] rounded-sm flex flex-col h-full">
            <div className="px-4 py-3 border-b border-[#e4eaee] flex items-center justify-between">
                <h2 className="text-[13px] font-normal text-[#555]">Most tracked activities</h2>
                <span className="text-[11px] font-semibold text-[#03a9f4]">Top {activities.length}</span>
            </div>
                ...
                {activities.map((activity, i) => (
                    <div key={i} className="px-4 py-3.5 border-b border-[#f5f5f5] flex items-start justify-between hover:bg-[#fafbfc] transition-colors cursor-default">
                        <div className="flex flex-col truncate pr-3 min-w-0">
                            <span className="text-[12px] font-medium text-[#333] truncate mb-1">{activity.title}</span>
                            <div className="flex items-center space-x-1.5 truncate">
                                <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: activity.projectColor }} />
                                <span className="text-[11px] text-[#999] truncate">{activity.project}</span>
                                {activity.lead && <span className="text-[11px] text-[#bbb] flex-shrink-0">- {activity.lead}</span>}
                            </div>
                        </div>
                        <span className="text-[12px] font-semibold text-[#333] tabular-nums ml-auto flex-shrink-0">{activity.time}</span>
                    </div>
                ))
                }
                {
