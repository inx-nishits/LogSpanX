...
import { Play, MoreVertical, Tag as TagIcon, DollarSign, Calendar, ListChecks, Check } from 'lucide-react'
import { 
  DropdownMenu, 
  DropdownMenuContent, 
  DropdownMenuItem, 
          ...
          <div key={date} className="relative group/day">
            <div className="flex items-center justify-between px-4 py-2 bg-[#f0f7fb] border border-[#d6e5ef] text-xs text-slate-600 rounded-t-sm">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="relative group ml-1 flex items-center">
                  <button onClick={() => setIsBulkMode(!isBulkMode)} className={cn("p-1.5 rounded transition-all cursor-pointer", isBulkMode ? "bg-[#03a9f4] text-white" : "text-gray-500 hover:bg-white hover:text-[#03a9f4] hover:shadow-sm")}>
                    <ListChecks className="h-[18px] w-[18px] stroke-[1.5px]" />
                  </button>
                  <div className="absolute top-full mt-1.5 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[11px] font-bold px-2 py-1 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                    Bulk edit
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white border-x border-b border-gray-200">
              {dayEntries.map((item, index) => {
                ...
                return (
                  <div key={item.id} className={cn("group flex items-center px-4 py-0 hover:bg-[#fcfdfe] transition-colors min-h-[54px] relative", index !== dayEntries.length - 1 && "border-b border-gray-100", isSelected && "bg-[#f2f9ff]")}>
                    {isBulkMode && (
                    ...
                    <div className="flex-[3] min-w-0 px-4 border-l border-dotted border-gray-200 self-stretch flex items-center"><ProjectPicker selectedProjectId={item.projectId} onSelect={(pid) => updateTimeEntry(item.id, { projectId: pid })} onClear={() => updateTimeEntry(item.id, { projectId: undefined })} /></div>
                      ...
                      <div className="flex items-center space-x-0 self-stretch flex-shrink-0">
                       <div className="flex items-center space-x-6 px-4 border-l border-dotted border-gray-200 h-full">
                          <TagPicker 
                            ...
                            onToggle={(tagId) => {
                              const newTags = item.tagIds.includes(tagId) 
                                ? item.tagIds.filter(id => id !== tagId) 
                                : [...item.tagIds, tagId];
                              updateTimeEntry(item.id, { tagIds: newTags });
                            }} 
                          />
                      ...
                      <div className="flex items-center space-x-1 text-[13px] text-gray-700 px-4 border-l border-dotted border-gray-200 h-full text-center">
                        <input type="text" defaultValue={formatLogSpanTime(startTime)} onBlur={(e) => {
                          const parsed = parseTimeAsDate(e.target.value, startTime);
                          if(parsed && !isNaN(parsed.getTime())) updateTimeEntry(item.id, { startTime: parsed });
                        }} className="w-[75px] text-center font-medium bg-transparent border border-transparent hover:border-gray-200 focus:border-[#03a9f4] focus:ring-1 focus:ring-[#03a9f4] rounded-sm py-0.5 outline-none uppercase transition-colors" />
                        <span className="text-gray-300">-</span>
                        <input type="text" defaultValue={endTime ? formatLogSpanTime(endTime) : '--:--'} onBlur={(e) => {
                          if (e.target.value === '--:--' || e.target.value.trim() === '') return;
                          const parsed = parseTimeAsDate(e.target.value, endTime || startTime);
                          if(parsed && !isNaN(parsed.getTime())) updateTimeEntry(item.id, { endTime: parsed });
                        }} className="w-[75px] text-center font-medium bg-transparent border border-transparent hover:border-gray-200 focus:border-[#03a9f4] focus:ring-1 focus:ring-[#03a9f4] rounded-sm py-0.5 outline-none uppercase transition-colors" />
                        <div className="ml-2">
                      ...
                      <div className="flex items-center space-x-2 px-4 border-l border-dotted border-gray-200 h-full">
                        <button onClick={() => startTimer(item)} className="p-1.5 text-gray-400 hover:text-[#03a9f4] transition-colors cursor-pointer"><Play className="h-[18px] w-[18px] stroke-[1.2px] fill-current opacity-60 hover:opacity-100" /></button>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild><button className="p-1.5 text-gray-400 hover:text-gray-700 transition-colors cursor-pointer"><MoreVertical className="h-4 w-4 stroke-[1.2px]" /></button></DropdownMenuTrigger>
